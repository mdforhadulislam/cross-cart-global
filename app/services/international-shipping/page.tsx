    ...
    <div className="min-h-screen bg-white pt-12 pb-20 px-5">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-4 mb-12">
          {SERVICES.map((s, i) => (
            <div key={i} className="bg-white/5 border border-gray-200 rounded-xl p-6 text-center">
              <s.icon className="h-10 w-10 mx-auto mb-3" style={{ color: s.color }} />
              <h3 className="text-gray-900 font-bold mb-1">{s.title}</h3>
              <p className="text-gray-900/50 text-sm">{s.desc}</p>
            </div>
          ))}
        </div>

        <div className="bg-white/5 border border-gray-200 rounded-2xl p-8 mb-8">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Popular International Routes</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {ROUTES.map((route, i) => (
              <div key={i} className="bg-white/5 rounded-lg p-3 text-center">
                <Globe className="h-5 w-5 text-[#F5B400] mx-auto mb-1" />
                <span className="text-gray-900/70 text-sm">{route}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-[#F5B400]/5 border border-[#F5B400]/20 rounded-xl p-6">
          <h3 className="text-gray-900 font-bold mb-3">What{"'"}s Included</h3>
          <div className="grid grid-cols-2 gap-2">
            {["Door-to-door delivery", "Customs clearance", "Real-time tracking", "Insurance options", "24/7 support", "Competitive rates"].map((item, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <span className="text-gray-900/60 text-sm">{item}</span>
              </div>
            ))}
          </div>
        </div>
